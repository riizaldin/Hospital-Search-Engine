...
def detail(hospital):
    ...
    WHERE {{
        ?subject schema:name "{hospital}".
        OPTIONAL {{ ?subject schema:tempat_tidur ?tempat_tidur . }}
        OPTIONAL {{ ?tempat_tidur schema:kelas_tempat ?kelas_tempat . }}
        OPTIONAL {{ ?tempat_tidur schema:kapasitas ?kapasitas . }}
    }}
    GROUP BY ?kelas_tempat ?kapasitas 
    """
    ...
    WHERE {{
        ?subject schema:name "{hospital}".
        OPTIONAL {{ ?subject schema:tenaga ?tenaga . }}
        OPTIONAL {{ ?tenaga schema:grup ?grup . }}
        OPTIONAL {{ ?tenaga schema:sdm ?sdm . }}
        OPTIONAL {{ ?tenaga schema:jumlah ?jumlah . }}
    }}
    GROUP BY ?grup ?sdm ?jumlah
    """
