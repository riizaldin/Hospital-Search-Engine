...
latitude_now = 0
longitude_now = 0
@app.route('/tes3', methods=["POST"])
def halo():
    global latitude_now, longitude_now
    # raise Exception("post diterima")
    ...
    asyncio.ensure_future(printkanla())
    return "halo"

async def printkanla():
    await asyncio.sleep(10)
    global latitude_now, longitude_now
    if latitude_now and longitude_now:
        # latitude_now = session.get('latitude', None)
        # longitude_now = session.get('longitude', None)
        print("KOOORDINATTTTTTTTTTTT : " + latitude_now + longitude_now)

@app.route('/tes2')
async def tes2():
    # asyncio.gather(printkanla())
    asyncio.run(printkanla())
    latitude_now = 0
    longitude_now = 0
    allQuery = f"""
    PREFIX schema: <http://websemantikkelompok6.org/>
    ...
    for name, coords in coordinates_dict.items():
        lat2, lon2 = map(float, coords)
        distance = haversine_distance(latitude_now, longitude_now, lat2, lon2)
        distances_dict[name] = distance
    ...
    return render_template("tes2.html", count=count, coordinates=coordinates_dict, longitude=1, latitude=2,distances=distances_dict, closest_hospitals_details=closest_hospitals_details)
